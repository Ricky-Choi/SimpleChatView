...
class ChatInputAccessoryView: UIInputView {
    ...
    override var intrinsicContentSize: CGSize {
        ...
        if newSize.height < ChatInputAccessoryView.preferredHeight {
            newSize.height = ChatInputAccessoryView.preferredHeight
        }
        if newSize.height > 120.0 {
            newSize.height = 120.0
        }
        return newSize
    }
    ...
    override init(frame: CGRect, inputViewStyle: UIInputView.Style) {
        ...
        expandingTextView.backgroundColor = UIColor(red: 1, green: 1, blue: 0, alpha: 0)
        expandingTextView.setContentHuggingPriority(.defaultHigh, for: .vertical)
